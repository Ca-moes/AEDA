...
void REAgency::generateCatalog() {
    for (auto i : properties){
        // Pertence a PropertyTypeItem existente se tiver a mesma morada que a existente e mesmo codigo postal
        bool found = false;
        BSTItrIn<PropertyTypeItem> it(catalogItems);
        while (!it.isAtEnd()){
            found = true;
        ...
        if(!found){
            PropertyTypeItem a = PropertyTypeItem(i->getAddress(),i->getPostalCode(), i->getTypology(), i->getPrice());
