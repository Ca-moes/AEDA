\hypertarget{_g_mock_faq_autotoc_md112}{}\doxysubsection{When I call a method on my mock object, the method for the real object is invoked instead. What\textquotesingle{}s the problem?}\label{_g_mock_faq_autotoc_md112}
In order for a method to be mocked, it must be {\itshape virtual}, unless you use the \href{\#MockingNonVirtualMethods}{\texttt{ high-\/perf dependency injection technique}}.\hypertarget{_g_mock_faq_autotoc_md113}{}\doxysubsection{Can I mock a variadic function?}\label{_g_mock_faq_autotoc_md113}
...
Ellipsis arguments are inherited from C and not really a C++ feature. They are unsafe to use and don\textquotesingle{}t work with arguments that have constructors or destructors. Therefore we recommend to avoid them in C++ as much as possible.\hypertarget{_g_mock_faq_autotoc_md114}{}\doxysubsection{M\+S\+V\+C gives me warning C4301 or C4373 when I define a mock method with a const parameter. Why?}\label{_g_mock_faq_autotoc_md114}
...
\end{DoxyCode}
\hypertarget{_g_mock_faq_autotoc_md115}{}\doxysubsection{I can\textquotesingle{}t figure out why g\+Mock thinks my expectations are not satisfied. What should I do?}\label{_g_mock_faq_autotoc_md115}
...
return type has no default value set.\char`\"{}, then try \href{for_dummies.md\#DefaultValue}{\texttt{ adding a default action}}. Due to a known issue, unexpected calls on mocks without default actions don\textquotesingle{}t print out a detailed comparison between the actual arguments and the expected arguments.\hypertarget{_g_mock_faq_autotoc_md116}{}\doxysubsection{My program crashed and $<$tt$>$\+Scoped\+Mock\+Log$<$/tt$>$ spit out tons of messages. Is it a g\+Mock bug?}\label{_g_mock_faq_autotoc_md116}
...
\end{DoxyCode}
\hypertarget{_g_mock_faq_autotoc_md117}{}\doxysubsection{How can I assert that a function is N\+E\+V\+E\+R called?}\label{_g_mock_faq_autotoc_md117}
...
\end{DoxyCode}
\hypertarget{_g_mock_faq_autotoc_md118}{}\doxysubsection{I have a failed test where g\+Mock tells me T\+W\+I\+C\+E that a particular expectation is not satisfied. Isn\textquotesingle{}t this redundant?}\label{_g_mock_faq_autotoc_md118}
...
By changing {\ttfamily $\sim$\+Base()} to virtual, {\ttfamily $\sim$\+Derived()} will be correctly called when {\ttfamily delete p} is executed, and the heap checker will be happy.\hypertarget{_g_mock_faq_autotoc_md120}{}\doxysubsection{The \char`\"{}newer expectations override older ones\char`\"{} rule makes writing expectations awkward. Why does g\+Mock do that?}\label{_g_mock_faq_autotoc_md120}
...
Also, you can control the verbosity by specifying {\ttfamily -\/-\/gmock\+\_\+verbose=error}. Other values are {\ttfamily info} and {\ttfamily warning}. If you find the output too noisy when debugging, just choose a less verbose level.\hypertarget{_g_mock_faq_autotoc_md122}{}\doxysubsection{How can I delete the mock function\textquotesingle{}s argument in an action?}\label{_g_mock_faq_autotoc_md122}
...
\end{DoxyCode}
\hypertarget{_g_mock_faq_autotoc_md123}{}\doxysubsection{How can I perform an arbitrary action on a mock function\textquotesingle{}s argument?}\label{_g_mock_faq_autotoc_md123}
...
\end{DoxyCode}
\hypertarget{_g_mock_faq_autotoc_md124}{}\doxysubsection{My code calls a static/global function. Can I mock it?}\label{_g_mock_faq_autotoc_md124}
...
This Google Testing Blog \href{https://testing.googleblog.com/2008/06/defeat-static-cling.html}{\texttt{ post}} says it excellently. Check it out.\hypertarget{_g_mock_faq_autotoc_md125}{}\doxysubsection{My mock object needs to do complex stuff. It\textquotesingle{}s a lot of pain to specify the actions. g\+Mock sucks!}\label{_g_mock_faq_autotoc_md125}
...
See this \href{cook_book.md\#mocking-side-effects}{\texttt{ recipe}} for more details and an example.\hypertarget{_g_mock_faq_autotoc_md129}{}\doxysubsection{I have a huge mock class, and Microsoft Visual C++ runs out of memory when compiling it. What can I do?}\label{_g_mock_faq_autotoc_md129}
