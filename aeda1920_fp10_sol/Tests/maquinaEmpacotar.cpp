...
Caixa MaquinaEmpacotar::procuraCaixa(Objeto& obj) {
	...
	while ( !caixas.empty() )
	{
		...
		if ( cx.getCargaLivre() >= obj.getPeso() ) {
			for (const auto & i : temp) //repoe na fila de caixas as caixas retiradas da frente.
		        caixas.push(i);
			return cx;
		}
		else temp.push_back(cx); //coloca as caixas nao utilizadas num vetor temporario
	}
	for (const auto & i : temp) //repoe as caixas nao utilizadas de volta, caso nenhuma sirva
		caixas.push(i);
